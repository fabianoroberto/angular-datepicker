        ...
        datepickerDirective = function datepickerDirective($window, $compile, $locale, $filter, $interpolate, $timeout) {
            var linkingFunction = function linkingFunction($scope, element, attr) {
                ...
                $scope.paginateYears = function paginateYears(startingYear) {
                    ...
                    //check range dates
                    $scope.paginationYearsNextDisabled = !!($scope.dateMaxLimit &&
                    theNewYears &&
                    theNewYears.length &&
                    !$scope.isSelectableMaxYear(Number(theNewYears[theNewYears.length - 1]) + 1));

                    $scope.paginationYearsPrevDisabled = !!($scope.dateMinLimit &&
                    theNewYears &&
                    theNewYears.length &&
